class BankAccount:
    ...
    def basic_details(self):
        print(f'User: {self.name}\t Account No: {self.cust_acc_num}\t Balance: Php {self.acc_balance}')

    def deposit(self):
        amount = int(input('Enter the deposit amount: '))
        if amount > 0:
            self.acc_balance      = self.acc_balance + amount
            print(f'Transaction completed. Current Balance: Php' '{self.acc_balance}')
        else:
...
if __name__ == '__main__':
    ...
    cust1 = BankAccount(name='John', mobile_no=9123456789, initial_depo=1000, pin=123)
    cust2 = BankAccount(name='Carlo', mobile_no=9567891011, initial_depo=2000, pin=456)
